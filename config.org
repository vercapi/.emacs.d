...
** Magit config
  ...
  Using magit for source control
  #+BEGIN_SRC emacs-lisp
    (use-package magit
      :ensure t
...
*** Core
   ...
   #+BEGIN_SRC emacs-lisp
     (defun multi-git-exec (rootdir action)
       ...
       (progn
         (with-current-buffer (get-buffer-create "*Mutlti git log*")
           (erase-buffer)
           (insert (concat "Root directory: " rootdir))
           (newline)
           (mapc (lambda (dir) (insert (concat "- " dir)) (newline))
                 (multi-git-dirs rootdir))
           (newline)
           (insert (concat "executing function: " (format "%s" action)))
           (newline 2)
           (mapc (lambda (dir)
...
*** Mode
    ...
    Currently this is not really a mode. More helper functions to use in the output
    #+BEGIN_SRC emacs-lisp
      (defun multi-git-open-magit-at-point ()
        ...
        (let ((rootdir (buffer-substring (point) (line-end-position)))
          ...
          (magit-status (concat rootdir "/" repo-name))))
    #+END_SRC
