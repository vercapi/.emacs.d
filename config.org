...
:AIINSTRUCTIONS:
`quelpa-use-package' registers the `:quelpa' keyword, so it must be demanded --
deferring it makes every `:quelpa' block fail to expand.  `quelpa' itself stays
deferred: `quelpa-use-package-inhibit-loading-quelpa' is what stops it being
required, and it must be set before the library is loaded, so it belongs in
this block and not later in the file.  With it enabled, a `:quelpa' recipe
expands to a no-op when the package is already in `package-user-dir', and still
falls back to `quelpa' when it is missing.
:END:
Git-sourced packages are treated as already installed at startup; refresh them
explicitly with =pve/quelpa-refresh=.
#+BEGIN_SRC emacs-lisp
  ...
  (defun pve/quelpa-refresh ()
    "Install or update all packages sourced from git through quelpa."
    (interactive)
    (let ((quelpa-use-package-inhibit-loading-quelpa nil))
      (require 'quelpa)
      (dolist (recipe
               '((copilot :fetcher github :repo "copilot-emacs/copilot.el" :branch "main"
                          :files ("copilot.el" "copilot-balancer.el" "copilot-nes.el"))
                 (mcp-server :fetcher github :repo "rhblind/emacs-mcp-server" :branch "main"
                             :files ("*.el" "tools/*.el" "mcp-wrapper.py" "mcp-wrapper.sh"))
                 (sunrise-commander :fetcher github :repo "escherdragon/sunrise-commander")))
        (quelpa recipe))
      (message "quelpa refresh done; restart Emacs to pick up new versions")))
#+END_SRC
