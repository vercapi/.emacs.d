...
We disable the built-in ~org-lint~ checker.  Since Org 9.7 it returns
each report's line as a propertized string (carrying an ~org-lint-marker~
text property) rather than a number, which flycheck's adapter feeds
straight into ~flycheck-error-new-at~, producing
=Wrong type argument: number-or-marker-p=.  org-lint is meant as an
on-demand report anyway — run =M-x org-lint= when a lint pass is wanted.
...
#+BEGIN_SRC emacs-lisp
  (use-package helm
    ...
    ;; Guard against a leaked `helm-M-x--timer'.  Its callback
    ;; `helm-M-x--notify-prefix-arg' runs `with-helm-window' whenever a
    ;; prefix arg is set; when the timer survives an abnormal helm exit
    ;; there is no live helm window, so `select-window' on nil signals
    ;; `(wrong-type-argument window-live-p nil)'.  This surfaces e.g. when
    ;; filtering the org agenda by TODO state.  Only notify when a helm
...
Prevents spurious `window-live-p' timer errors from a leaked
`helm-M-x--timer'."
      (when (helm-window)
        (apply orig-fn args)))
