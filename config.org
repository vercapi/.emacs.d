...
*** Git operators 
    ...
    #+BEGIN_SRC emacs-lisp
      (defun multi-git-show-current-branch-name ()
        ...
        (list
         (format "Current branch: %s" (magit-get-current-branch))
         (format "changes: %s" (shell-command-to-string "git status --porcelain=v1 2>/dev/null | wc -l"))
         (format "Unpushed: %s" (shell-command-to-string (format "git log origin/%s..HEAD | grep commit | wc -l" (magit-get-current-branch))))))
    #+END_SRC
    ...
    #+BEGIN_SRC emacs-lisp
      (defvar multi-git-mode-map
        (let ((map (make-keymap)))
          ...
          (define-key map "p" 'multi-git-push-to-remote-at-point)
          map)
...
*** Workflow states
    ...
    #+BEGIN_SRC emacs-lisp
      (setq org-todo-keywords
            '((sequence "TODO(t)" "DOING(o!)" "BLOCKED(b@/!)" "WAITING(e@!)" "|" "DONE(d!)" "CANCELED(c@)")
