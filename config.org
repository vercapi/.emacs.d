...
** Custom key  bindings
  ...
  A simple toggle function for org headers to be used in the hydra
  #+BEGIN_SRC emacs-lisp
    (defun pve-org/toggle-subtree ()
      "Toggle subtree in org mode"
      (interactive)
      (if (org-at-heading-p)
          (if (org-subtree-end-visible-p)
