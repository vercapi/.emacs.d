...
*** File setup
   ...
   #+BEGIN_SRC emacs-lisp
     ...
     (defvar org-default-documents (concat org-default-dir "documents.org") "Directory for documents such as PDFs.")
...
*** Attachments & files
   ...
   Move files to location and link them into the CMS org file.
   #+BEGIN_SRC emacs-lisp
   
   #+END_SRC

   Generate preview image from PDF and link into the CMS org file.
   #+BEGIN_SRC emacs-lisp
   
   #+END_SRC

   Get al the headers in the CMS org file
   #+BEGIN_SRC emacs-lisp
     ;; Need to include this for helm-source-org-headings-for-files
     (require 'org-helm)
     (defun cms-headers ()
       "Find org heads for storing documents"
       (interactive)
       (with-temp-buffer
         (org-mode)
         (helm :sources (helm-source-org-headings-for-files (list org-default-documents))
